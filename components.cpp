...
LRESULT CALLBACK AkkadianEditControl(
	HWND hwnd,
	UINT msg,
	WPARAM w_param,
	LPARAM l_param,
	UINT_PTR u_id_subclass,
	DWORD_PTR dw_ref_data
) {
	UNREFERENCED_PARAMETER(u_id_subclass);
	UNREFERENCED_PARAMETER(dw_ref_data);

    switch (msg) {
		case WM_KEYDOWN: {
			if (w_param == VK_UP) {
				int res = (int)SendMessage(hwnd, EM_GETSEL, NULL, NULL);
